# Motor IO Interfaces Across the Corpus — Survey & a Proposed Unified Contract
...
reusable one, dissects the design axes they disagree on, and proposes a single contract
that subsumes every use case found.
...
## 5. A proposed unified contract

Design goals, distilled from the six:

1. **One interface, every mechanism** (254/2910): rollers, flywheels, arms, elevators,
   pivots, wrists, climbers — distinguished only by which methods they implement.
2. **`double` mechanism units on the hot path** (254/2910/5137 majority) — fastest, no
   allocation, simplest sim; the unit convention is *documented*, not type-enforced. (We
   trade 971/1678's compile-time unit safety for simplicity; a typed façade can wrap it.)
3. **Loud, not silent, on unsupported** (5137's lesson) — but make it *queryable* up front
   via a capability set, so a subsystem fails fast at construction, not mid-match.
4. **Reified `Request`** (1678/2706's lesson) as an *optional* control path — immutable,
   storable, replayable, and able to carry a transient current/voltage limit — with plain
   setters delegating to it so simple subsystems never touch it.
5. **One flat `@AutoLog` inputs struct** covering the consensus + introspection fields,
   logging both motor-frame and mechanism-frame (2910).
6. **Lifecycle in a base class** (1678/2706): `enable()/disable()` with last-request reapply
   and dashboard alerts (5137) live in an optional `MotorIOBase`, keeping the interface pure.
...
And the optional stateful base that supplies 1678/2706's lifecycle and 5137's alerts —
this is where `enable()/disable()`, "reapply last request," and dashboard `Alert`s live, so
the interface itself stays pure:
...
### How it covers each team's use cases
...
- **Doubles over `Units`.** We follow the 4-of-6 majority for hot-path speed and sim
  simplicity, accepting that unit correctness is now a *convention* (enforced by review /
  the build-spec), not the compiler. Teams that prize 971/1678's safety can add a typed
  wrapper over this core without changing implementations.
- **No vendor types in the signature.** Unlike 254 (`NeutralModeValue`,
  `MotionMagicConfigs`), 2910 (`getTalon()`), 5137 (`Slot0Configs`, `GravityTypeValue`),
  this contract exposes only `frc.lib.io` types and primitives — so `com.ctre`/
  `com.revrobotics` stays *below* the IO line, satisfying the rubric's hard rule.
...
## 6. Appendix — file locations (corpus)
