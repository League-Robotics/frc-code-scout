...
### Query the corpus in SQL (`index-db` → DuckDB)

`index-db` parses every source file with **tree-sitter** and loads the whole corpus into one
**DuckDB** database (`data/code-index.duckdb`, gitignored). Needs the `index` extra:

```bash
uv venv --python 3.13 && uv pip install -e '.[index]'
uv run python main.py index-db --rebuild        # parse all 63 teams -> DuckDB
uv run python main.py index-db --team 1155 --rebuild --db /tmp/one.duckdb   # one team
```

Tables: `symbols`, `files`, `imports` (the code analysis — **every row carries `team`,
`year`, `repo`, `source_file`, `file_path`**), plus `teams`, `epa`, `repos`, `commits`,
`commit_files` from the corpus metadata, so code joins to competition results:

```sql
-- IO-layer adoption: interface declarations named *IO, by team/year (no joins)
SELECT team, year, repo, name FROM symbols
WHERE kind='interface' AND name LIKE '%IO' ORDER BY team, year;

-- code vs results: method count vs 2025 normalized EPA per team
SELECT t.name, count(*) FILTER (WHERE s.kind='method') AS methods,
       max(e.norm_epa) FILTER (WHERE e.year=2025) AS epa25
FROM symbols s JOIN teams t ON t.team=s.team LEFT JOIN epa e ON e.team=s.team
GROUP BY t.name ORDER BY epa25 DESC NULLS LAST;
```

`index-db` is lazy-loaded, so the stdlib-only commands above still run without the extra.

### The analysis notebook

`notebooks/frc-code-analysis.ipynb` presents the findings — seasonality of team effort,
framework adoption by season, an automated D1–D8 rubric pass validated against the
hand-scored teams, testing/simulation coverage, and code-vs-EPA correlations. Regenerate it
(charts + tables embedded) from the DuckDB index with:

```bash
uv pip install -e '.[index,notebook]'
uv run python scripts/build_notebook.py     # -> notebooks/frc-code-analysis.ipynb
```
